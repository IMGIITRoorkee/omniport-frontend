...
read -p "Enter app name with spaces: " name
...
# Replace placeholders with app related data
cd src/components
sed -i "s/\[\[app\-display\-name\]\]/${app_name_display}/g" app.js
printf "Added ${app_name_display} in header\n"
sed -i "s/\[\[app_name\]\]/${app_name_underscore}/g" app.js
printf "Added ${app_name_underscore} in placeholder text\n"
git add ../../src
